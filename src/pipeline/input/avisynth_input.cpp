...
bool AviSynthInput::loadLibrary(const std::string& customLibPath) {
    ...
    if (!customLibPath.empty()) {
        libHandle_ = LoadLibraryA(customLibPath.c_str());
    } else {
        libHandle_ = LoadLibraryA("AviSynth.dll");
        if (!libHandle_) libHandle_ = LoadLibraryA("avisynth.dll");
    }
...
#define LOAD_FUNC(name) \
    ...
    // Optional pointer extraction functions
...
bool AviSynthInput::open(const std::string& path) {
    ...
    std::string customLib;

    // Check for custom dll prefix format: [custom.dll]://script.avs
    if (path[0] == '[') {
        size_t pos = path.find("]://");
        if (pos != std::string::npos) {
            customLib = path.substr(1, pos - 1);
            scriptPath = path.substr(pos + 4);
        }
    }

    if (!loadLibrary(customLib)) {
        return false;
    }
    ...
    AVS_Value args = avs_new_value_string(scriptPath.c_str());
    AVS_Value res = func_.avs_invoke(env_, "Import", args, nullptr);
...
std::optional<VideoFrame> AviSynthInput::readFrame() {
    ...
    // Copy planar data if accessor functions are available
    if (func_.avs_get_read_ptr_p && func_.avs_get_pitch_p) {
        for (int p = 0; p < planeCount_; ++p) {
            const uint8_t* src = func_.avs_get_read_ptr_p(avsFrm, p);
            int srcPitch = func_.avs_get_pitch_p(avsFrm, p);
            int h = (p == 0 || info_.colorSpace == 2) ? info_.height : (info_.height / 2);
            int bytesPerSample = (info_.bitDepth > 8) ? 2 : 1;
            int rowSize = (p == 0 || info_.colorSpace == 2) ? (info_.width * bytesPerSample) : ((info_.width / 2) * bytesPerSample);

            uint8_t* dst = const_cast<uint8_t*>(frame.planes[p].data());
            for (int y = 0; y < h; ++y) {
                std::memcpy(dst + y * frame.strides[p], src + y * srcPitch, rowSize);
            }
        }
    }
