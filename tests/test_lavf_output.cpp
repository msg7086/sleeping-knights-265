...
TEST_CASE("LavfOutput handles invalid destination gracefully", "[output][lavf]") {
    sk265::pipeline::output::LavfOutput output;
...
TEST_CASE("LavfOutput multiplexes HEVC to MKV container cleanly", "[output][lavf]") {
    std::string testPath = "test_lavf_mux.mkv";
...
TEST_CASE("LavfOutput multiplexes HEVC to MP4 container cleanly", "[output][lavf]") {
    std::string testPath = "test_lavf_mux.mp4";
    if (std::filesystem::exists(testPath)) {
        std::filesystem::remove(testPath);
    }
